package com.epam.lab.mentoring;

public final class ApplicationConstants {
    public static final String CLASSES_OUTPUT_TEMPLATE = "%s(%s)";
    public static final String EXTENSION_FOLDER = "plugins/";

    public static final String QUIT_COMMAND = "quit";

    public static final String LOAD_CLASSES_CMD = "1";

    public static final String TEST_CLASS_CMD = "2";

    public static final String CONSOLE_TEMPLATE = "\nAvailable actions ==>\n" +
            "Show loaded classes: 1\n" +
            "Invoke class: 2\n";

    public static final String PLUGIN_EXTENSION = "jar";
    public static final String PLUGIN_EXTENSION_WITH_POINT = ".jar";

    public static final String CLASS_FILE_EXTENSION = ".class";
}
